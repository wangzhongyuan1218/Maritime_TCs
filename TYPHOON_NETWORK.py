...
def single_process(file_item):
    filename = os.path.split(file_item)[1].split(".")[0]
    file = pd.read_csv(file_item)
    if len(file) == 0:
        return
    file['FLAG'] = False
    file = file.drop_duplicates(subset=['FROM', 'DESTINATION', 'SHIPTYPE'], keep='last', inplace=False)  # 去重复
    date = str(file.date.values[0])
    month = date[4:6]
    month = int(month) - 1
    affected_static_network = static_network[month]
    affected_static_network = affected_static_network.merge(file, how='left',
                                                            on=['FROM', 'DESTINATION', 'SHIPTYPE'])
    affected_static_network['FLAG'] = affected_static_network['FLAG'].fillna(True)
    affected_static_network = affected_static_network[affected_static_network['FLAG'] == True]
    for item in SHIPTYPE:
        shapefile = r"D:\AIS_TEMP\17\DEGREE_{0}_{1}.shp".format(filename, item)
        if os.path.exists(shapefile):
            print(shapefile)
            continue
        print(shapefile+":正在处理")
        if item == "ALL":
            TYPE = ALL
        elif item == "FISHING":
            TYPE = FISHING
...
if __name__ == '__main__':
    ...
    for item in file_list:
        ...
        month=int(month)-1
        affected_static_network=static_network[month]
        affected_static_network=affected_static_network.merge(file,how='left',on=['FROM','DESTINATION','SHIPTYPE'])
        affected_static_network['FLAG']=affected_static_network['FLAG'].fillna(True)
        affected_static_network=affected_static_network[affected_static_network['FLAG']==True]
        for item in SHIPTYPE:
            shapefile = r"D:\AIS_TEMP\17\DEGREE_{0}_{1}.shp".format(filename, item)
